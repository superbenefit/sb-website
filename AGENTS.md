...
Public website for SuperBenefit — an Astro 5 site deployed to Cloudflare Workers. Presents SuperBenefit's work across governance, DAOs, and systems innovation through a visual storytelling homepage with themed content sections.
...
Hero → SectionDivider (wave) → MissionStatement → CardsStage (dark charcoal wrapper containing three StorySection cards) → CallToAction.

**StorySection** (`src/components/StorySection.astro`) is the core parameterized component. Accepts `theme` prop (`'green' | 'orange' | 'red'`), background image, and content data. Each instance renders as an elevated card (20px border-radius, box-shadow, 3px accent-colored top stripe) floating over the dark CardsStage. Cards have 48px vertical gaps; first/last cards overlap into adjacent sections by ~60px.

StorySection layout variants via `variant` prop:
...
- **Reveal**: `data-animate="fade-up|fade-left|fade-right|scale-in"` triggers CSS `animation-timeline: view()`
- **Parallax**: `data-parallax` on background elements, GPU-composited via `transform: translateY()`
- **Header solidify**: Transparent → blurred over first 300px via `animation-timeline: scroll()`
